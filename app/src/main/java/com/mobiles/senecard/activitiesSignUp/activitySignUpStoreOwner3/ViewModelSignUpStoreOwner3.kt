        ...
        viewModelScope.launch {
            ...
            if (isScheduleValid) {
                if (repositoryAuthentication.createUser(signUpUser.email!!, signUpUser.password!!)) {
                    if (repositoryUser.addUser(name = signUpUser.name!!, email = signUpUser.email!!, phone = signUpUser.phone!!, qrCode = "", role = "businessOwner") ) {
                        val user = repositoryUser.getUser(signUpUser.email!!)
                        if (user != null) {
                            if (repositoryStore.addStore(userId = user.userId, name = signUpStore.name!!, category = signUpStore.category!!, address = signUpStore.address!!, image = signUpStore.image!!, schedule = storeSchedule)) {
                                signUpUser.reset()
                                signUpStore.reset()
